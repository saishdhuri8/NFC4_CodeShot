...
const Dashboard = () => {
    ...
    const handleJoinInterview = async (e) => {
        ...
        navigate(`/room/${joinCode.trim()}?userId=${user.email}&role=candidate`);

        setJoinMessage({
            type: "success",
            text: `Joined interview with code: ${joinCode.trim()}`,
        });
        setJoinCode("");
    };
            ...
            >
                ...
                <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
                    <motion.button
                        ...
                        className={`w-full flex items-center gap-3 p-2 rounded-lg text-sm transition-colors ${activeTab === "interview" ? "bg-gray-700 text-blue-400" : "text-gray-400 hover:bg-gray-700 hover:text-gray-200"
                            }`}
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                    >
                        <div className="w-6 h-6 flex items-center justify-center">
                            <FaUserFriends className={activeTab === "interview" ? "text-blue-400" : "text-gray-400"} />
                        </div>
                    ...
                    <motion.button
                        ...
                        className={`w-full flex items-center gap-3 p-2 rounded-lg text-sm transition-colors ${activeTab === "playback" ? "bg-gray-700 text-purple-400" : "text-gray-400 hover:bg-gray-700 hover:text-gray-200"
                            }`}
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                    >
                        <div className="w-6 h-6 flex items-center justify-center">
                            <FaClock className={activeTab === "playback" ? "text-purple-400" : "text-gray-400"} />
                        </div>
                        {sidebarOpen && <span>Code Playback</span>}
                    </motion.button>
                </nav>
                ...
                >
                    <div>
                        <h1 className="text-xl font-bold text-gray-200">
                            {activeTab === "interview" ? "Interview Dashboard" : "Code Playback"}
                        </h1>
                        <p className="text-sm text-gray-500">
                            {activeTab === "interview" ? "Join or conduct technical interviews" : "Review your coding sessions"}
                        </p>
                    </div>

                    <div className="flex items-center gap-4">
                        <div className="text-sm text-gray-400 hidden md:block">Welcome, Saish</div>
                        <motion.div
                        ...
                        >
                            <img
                                src="https://ui-avatars.com/api/?name=Saish+Dhuri&background=0D8ABC&color=fff"
                                alt="User avatar"
                                ...
                                >
                                    ...
                                    <form onSubmit={handleJoinInterview} className="space-y-4">
                                        ...
                                        <AnimatePresence>
                                            {joinMessage && (
                                                <motion.div
                                                    className={`text-sm mt-2 p-3 rounded-lg ${joinMessage.type === "success"
                                    ...
                                    <form onSubmit={handleStartInterview} className="space-y-4">
                                        <div>
                                            ...
                                            <motion.textarea
                                                ...
                                                placeholder="Example: Assess front-end skills — 3 questions: 1) React state vs props, 2) build a small layout, 3) debugging scenario..."
                                                rows={8}
                                                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all text-gray-200"
                                        ...
                                        <AnimatePresence>
                                            {promptSaved && (
                                                <motion.div
                                                    className={`text-sm mt-2 p-3 rounded-lg ${promptSaved.type === "success"
                                                        ? "bg-green-900 text-green-400 border border-green-800"
                                                        : "bg-red-900 text-red-400 border border-red-800"
                                                        }`}
                                                    role="status"
                                        ...
                                        {promptSaved?.type === "success" && (
                                            <motion.div
                                                className="mt-4 border border-gray-700 rounded-lg p-4 bg-gray-800 text-sm"
                                                initial={{ opacity: 0, height: 0 }}
                                                animate={{ opacity: 1, height: "auto" }}
                                                transition={{ duration: 0.3 }}
                                            >
                                                <strong className="block mb-2 text-gray-300">Prompt Preview</strong>
                                                <div className="whitespace-pre-wrap text-gray-300 bg-gray-700 p-3 rounded border border-gray-600">
                                                    {prompt}
                                                </div>
                                            </motion.div>
                                        )}
